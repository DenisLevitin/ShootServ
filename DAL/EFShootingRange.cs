    ...
    public class EFShootingRange
    {
        ...
        public ShootingRangeParams Get(int idShootingRange)
        {
            ...
            using (var db = DBContext.GetContext())
            {
                try
                {
                    res = Convert(db.ShootingRanges.Where(x => x.Id == idShootingRange).First());
                }
                catch (Exception exc)
                {
        ...
        public ResultInfo Delete(int idShootRange)
        {
            ...
            try
            {
                using (var db = DBContext.GetContext())
                {
                    var deleting = db.ShootingRanges.Where(x => x.Id == idShootRange).Single();
